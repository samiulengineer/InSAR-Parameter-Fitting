...
def input_y_test(stack_size, low_limit, high_limit, experiment):
    ...
    x1 = x2 = torch.from_numpy(np.random.uniform(low_limit, high_limit, (stack_size, 1)).astype(np.float32)) * \
        torch.from_numpy(
    ...
    if (experiment == 1):
        ...
        a = b = torch.from_numpy(np.random.uniform(low_limit, high_limit, (stack_size, 1)).astype(
            np.float32)) * torch.from_numpy(np.random.uniform(low_limit, high_limit, (stack_size, 1)).astype(np.float32)) * 2
