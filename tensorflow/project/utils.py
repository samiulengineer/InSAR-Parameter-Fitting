...
class SelectCallbacks(keras.callbacks.Callback):
    ...
    def get_callbacks(self, val_dataset, model):
        ...
        if self.config['checkpoint']:
            self.callbacks.append(keras.callbacks.ModelCheckpoint(os.path.join(self.config['checkpoint_dir'], self.config['checkpoint_name']), save_best_only = True, save_weights_only=True))
...
def display(display_list, idx, directory, score, exp):
    ...
    for i in range(len(display_list)):
        ...
        plt.title(title[i])
        plt.imshow((display_list[title[i]]), cmap="rainbow", vmin=-5, vmax=5)
        plt.axis('off')

    prediction_name = "img_ex_{}_{}_Loss_{:.4f}.png".format(exp, idx, score) # create file name to save
...
def show_predictions(dataset, model, config, val=False):
    ...
    for feature, mask in data: # save all image prediction in the dataset
        ...
        for i in range(len(feature)): # save single image prediction in the batch
            ...
            score = m(mask[i], prediction[i]).numpy()
            
            display({"MR": np.squeeze(mask[i].numpy(), axis=-1),
                      "Prediction (loss_{:.4f})".format(score): np.squeeze(prediction[i], axis=-1)
